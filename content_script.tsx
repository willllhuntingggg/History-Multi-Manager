...
const fetchProjects = async () => {
  ...
  subMenuItems.forEach(el => {
    const title = el.querySelector('.truncate')?.textContent; // innerText is better but textContent is safer in some contexts
    if (title && title !== '新项目') {
      projects.push(title);
    }
  });
...
const initOverlay = () => {
  ...
  if (refreshBtn) refreshBtn.onclick = () => { scannedItems = scanHistory(); renderDashboard(); };
...
export {}; // Make this a module to avoid global scope conflict
